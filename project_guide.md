...
**mogaha_registry 현황 (2026-06-11 풀스캔 갱신):**
- 전체: **125,069건** (2026-06-07 123,841 → +1,228)
- 폐업: 48,212건
- 최신 개원일: 2026-06-09 / 최신 폐업일: 2026-06-21
- 영업중 + 좌표 있는 의원: **37,506건** (지도 표시 가능)

**(이전) mogaha_registry 현황 (2026-06-07):**
- 전체: 123,841건
...
### ✅ 행안부 API 풀스캔 갱신 + 폐원 조회 버그 수정 (2026-06-11)

**1) 폐원 지도 조회 버그 수정 (RPC v3)**
- 문제: 폐원 모드에서도 `year_group`(개원일 기준)으로 필터 → "26.4 폐원" 검색 시
  개원일이 아닌 폐업일 기준으로 나와야 하는데 개원일(25년 등)로 조회돼야만 나왔음
- 해결: `get_map_clinics_json()` RPC를 v3으로 교체
  - 개원/전체 모드: 기존처럼 `year_group`(license_date 기준) 매칭
  - **폐원 모드: `closed_date`의 연/월을 year_group 포맷("26.4")으로 변환해 매칭**
- SQL: `연간개원지도/supabase/rpc_get_map_clinics_v3.sql` (Supabase에서 실행 완료)
- 검증: 26.4 폐원 조회 → closed_date 2026-04인 의원만 정상 반환 확인

**2) 행안부 API 전체 풀스캔 갱신**
- `fill_mogaha_registry.py`로 전체 1,251페이지 재수집 → 125,069건
- **멈춤 원인 규명**: 행안부 API가 특정 페이지(p192, p583)에서 간헐적 400 Bad Request 반환
  → 기존 코드는 이때 전체 루프 중단 → "자꾸 멈추는" 현상의 진짜 원인
- **스크립트 2곳 수정 (`16.월별개원흐름/폐업자료/fill_mogaha_registry.py`):**
  - `upsert_batch()`: 배치 내 mogaha_id 중복 제거 → `ON CONFLICT cannot affect row a second time` 오류 제거 (1건씩 재시도로 느려지던 문제 해결)
  - `main()` 루프: 400 오류 시 **전체 중단 대신 해당 페이지 건너뛰고 계속 진행** + 건너뛴 페이지 목록 출력(재시도용)

**증분 수집이 불가능한 이유 (폐업 누락 방지):**
- 행안부 API에 날짜 필터 파라미터 없음 + 데이터 비정렬
- 폐업은 closed_date만 최근이고 license_date는 옛날 → license_date 기준 증분 시 **옛날 개원 의원의 신규 폐업이 통째로 누락**
- → 폐업 정보 정확성을 위해 **전체 페이지 순회(풀스캔)는 필수**
